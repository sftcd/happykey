...
# encrypt
$VALGRIND $BINDIR/hpkemain -e -P $TMPNAM.pub -i $TMPNAM.plain -o $TMPNAM.cipher -m psk -s $GOODPSK -n $GOODPSKID

# check decryption fails as expected
echo "Good psk: $GOODPSK pskid $GOODPSKID"
...
do
    ...
    do
        if [[ "$VALGRIND" == "" ]]
        then
            $BINDIR/hpkemain -d -p $TMPNAM.priv -i $TMPNAM.cipher -o $TMPNAM.recovered -m psk -s $psk -n $pskid 2>/dev/null
        else
            $VALGRIND $BINDIR/hpkemain -d -p $TMPNAM.priv -i $TMPNAM.cipher -o $TMPNAM.recovered -m psk -s $psk -n $pskid 
        fi
        res=$?
        if [[ "$res" == "0" ]]
        then
        ...
        echo "$psk $pskid results in $res/$strres"
    done
done
